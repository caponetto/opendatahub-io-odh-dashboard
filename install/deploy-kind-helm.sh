...
if [ "${PLUGINS_AVAILABLE}" = "true" ]; then
  ...
  for img in "${MR_SERVER_IMAGE}" "${PG_IMAGE}" "${BUSYBOX_IMAGE}" "${PERSES_IMAGE}" "${PROM_IMAGE}" "${THANOS_IMAGE}" "${MLMD_IMAGE}" "${ENVOY_IMAGE}"; do
  ...
  # Even without plugins, ensure mock infrastructure images are available
  for img in "${PERSES_IMAGE}" "${PROM_IMAGE}" "${THANOS_IMAGE}" "${MLMD_IMAGE}" "${ENVOY_IMAGE}"; do
    if ! ${CONTAINER_ENGINE} exec "${KIND_CLUSTER_NAME}-control-plane" crictl images -o json 2>/dev/null | grep -q "$(echo "${img}" | cut -d: -f1)"; then
      echo ">>> Pulling ${img}..."
      ${CONTAINER_ENGINE} pull "${img}"
      echo ">>> Loading ${img} into Kind..."
