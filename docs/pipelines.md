...
- From a project, the Pipelines section is rendered under `packages/workbenches/src/pages/screens/detail/pipelines/PipelinesSection.tsx`.
...
| `packages/dashboard-foundation-frontend/src/concepts/projects/ProjectsContext` | Frontend | Namespace resolution and preferred-project redirect |
| `packages/automl` / `packages/autorag` | Package | Same KFP/MLMD stack; runs and artifacts surface in Pipelines |
| `packages/workbenches/src/pages` | Frontend | `PipelinesSection` embeds list + server configuration; reuses pipeline concepts |
...
- **Tables:** Use the shared `Table` component from `packages/dashboard-foundation-frontend/src/components/table/` for pipeline/run/experiment/artifact lists. Do **not** import `TableComposable` directly in new pipeline tables.
