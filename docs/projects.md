...
- Spawner and permissions flows live under `packages/workbenches/src/pages/` alongside detail screens and follow the same routing boundaries.
...
- **Tables:** Resource tables on Workbenches, Storage, and Connections use the standard `Table` component from `packages/dashboard-foundation-frontend/src/components/table/Table.tsx`. `GenericHorizontalBar` itself uses stock PatternFly `Tabs` (no tab override).
