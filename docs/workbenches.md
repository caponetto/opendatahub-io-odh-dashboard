...
- Shared notebook helpers live under `packages/dashboard-foundation-frontend/src/concepts/notebooks/` and `packages/notebook-controller` so both entry points stay consistent.
...
- `useCheckJupyterEnabled` in `packages/notebook-controller/src/utilities/notebookControllerUtils.ts` reads `dashboardConfig.spec.notebookController.enabled` and sends users to `/` when disabled.
...
- Fetches use `useFetchState` and `packages/notebook-controller/src/api/k8s/notebooks.ts`, with status merged into `NotebookState[]` for presentation.
...
| **POLL_INTERVAL** | Shared poll cadence (`packages/dashboard-foundation-frontend/src/utilities/const.ts`) for live notebook/pod status. |

## Interactions

| Dependency | Type | Details |
|-----------|------|---------|
| `packages/notebook-controller` | Package | Shared utilities and types for spawner and controller |
...
- **Tables:** Import `Table` from `#~/components/table` (in `packages/dashboard-foundation-frontend/src/components/table/`), **not** `@patternfly/react-table` directly.
