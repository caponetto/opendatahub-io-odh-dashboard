...
- **Admin** decided via `SelfSubjectAccessReview` against the dashboard `Auth` CR (`packages/dashboard-foundation-backend/src/adminUtils.ts` / `authUtils.ts`), not the deprecated OpenShift Group API
...
- **HTTP prohibited in production**: `packages/dashboard-foundation-backend/src/httpUtils.ts` rejects `http:` proxy targets unless `APP_ENV=development` (`ProxyError` / `SETUP_FAILURE`)
- **Group API deprecated for BYO OIDC**: `adminUtils.ts` Group helpers are `@deprecated`; prefer SSAR via `createSelfSubjectAccessReview`
