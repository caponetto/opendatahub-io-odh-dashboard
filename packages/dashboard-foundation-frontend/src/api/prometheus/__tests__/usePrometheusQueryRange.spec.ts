...
import { PrometheusQueryRangeResponseData } from '@odh-dashboard/dashboard-foundation-frontend/types';
...
describe('usePrometheusQueryRange', () => {
  ...
  it('should fetch data successfully and handle refresh ', async () => {
    ...
    expect(responsePredicate).toHaveBeenCalledTimes(1);
    ...
    expect(responsePredicate).toHaveBeenCalledWith(mockedResponse.data.response.data);
    expect(responsePredicate).toHaveBeenCalledTimes(2);
