...
import { configureK8sClient, resetK8sClientConfig } from '@odh-dashboard/k8s-browser';
...
describe('addOwnerReference', () => {
  it('should not add any owner reference for undefined owner', () => {
    const target = addOwnerReference(resource);
  ...
  it('should not add owner reference when apiVersion or kind is not present', () => {
    const ownerWithoutApiVersion = mockProjectK8sResource({});
    ownerWithoutApiVersion.apiVersion = undefined;

    const ownerWithoutKind = mockProjectK8sResource({});
    ownerWithoutKind.kind = undefined;

    expect(addOwnerReference(resource, ownerWithoutApiVersion).metadata.ownerReferences).toEqual(
      [],
    );
    expect(addOwnerReference(resource, ownerWithoutKind).metadata.ownerReferences).toEqual([]);
  });
...
describe('k8sMergePatchResource', () => {
  afterEach(() => {
    resetK8sClientConfig();
  });

  it('should prepend the Kubernetes proxy prefix before issuing merge patch requests', async () => {
    const response = {
      status: 200,
      text: async () =>
        JSON.stringify({
    ...
    await k8sMergePatchResource({
      model: SecretModel,
      resource: mockSecretK8sResource({ name: 'example', namespace: 'test-project' }),
    });

    expect(mockFetch).toHaveBeenCalledWith(
      '/api/k8s/api/v1/namespaces/test-project/secrets/example',
