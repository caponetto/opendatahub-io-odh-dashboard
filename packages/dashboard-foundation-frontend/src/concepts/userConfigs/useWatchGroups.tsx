...
import { useGroups } from '#~/api/k8s/groups';
...
export const useWatchGroups = (): {
  ...
  const normalizedGroupsData = React.useMemo(() => groupsData ?? [], [groupsData]);
  ...
  React.useEffect(() => {
    const fetchGroups = () => {
      setIsLoading(true);
      fetchAuthGroups(normalizedGroupsData)
        .then((groupConfig) => {
  ...
  }, [notification, hasDirectAccessDataLoaded, normalizedGroupsData]);

  const updateGroups = React.useCallback(
    (group: GroupsConfig) => {
      setIsLoading(true);
      updateAuthGroups(group, normalizedGroupsData)
        .then((groupsResponse) => {
    ...
    [normalizedGroupsData, notification],
  );
