...
/**
 * Load .env files for an assembler package and set the derived _ODH_* vars
 * that the webpack configs expect.
 *
 * @param {Object} opts
 * @param {string} opts.assemblerDir - Absolute path to the assembler package root
 * @param {string} [opts.env] - Environment name (e.g. 'development', 'production')
 */
const setupAssemblerDotenv = ({ assemblerDir, env }) => {
  const REPO_ROOT = path.resolve(assemblerDir, '..', '..');

  // dotenv does NOT override existing vars, so the first file to set a
  // variable wins.  Load most-specific first: .local > env-specific > base,
  // repo-root scope before assembler scope.
  const envFiles = [
    ...(env
      ? [path.resolve(REPO_ROOT, `.env.${env}.local`), path.resolve(REPO_ROOT, `.env.${env}`)]
      : []),
    path.resolve(REPO_ROOT, '.env.local'),
    path.resolve(REPO_ROOT, '.env'),
    ...(env
      ? [path.resolve(assemblerDir, `.env.${env}.local`), path.resolve(assemblerDir, `.env.${env}`)]
      : []),
    path.resolve(assemblerDir, '.env.local'),
    path.resolve(assemblerDir, '.env'),
  ];
  ...
  process.env._ODH_RELATIVE_DIRNAME = assemblerDir;
  process.env._ODH_IS_PROJECT_ROOT_DIR = 'false';
  process.env._ODH_SRC_DIR = SRC_DIR;
  process.env._ODH_COMMON_DIR = COMMON_DIR;
  process.env._ODH_DIST_DIR = DIST_DIR;
  process.env._ODH_IMAGES_DIRNAME = process.env.ODH_IMAGES_DIRNAME || 'images';
  process.env._ODH_PUBLIC_PATH = process.env.ODH_PUBLIC_PATH || '/';
  process.env._ODH_HOST = process.env.ODH_HOST || 'localhost';
  process.env._ODH_PORT = process.env.ODH_PORT || process.env.FRONTEND_PORT || '3000';
  process.env._BACKEND_PORT = process.env.PORT || process.env.BACKEND_PORT || '8080';
};

module.exports = { setupWebpackDotenvFilesForEnv, setupDotenvFilesForEnv, setupAssemblerDotenv };
