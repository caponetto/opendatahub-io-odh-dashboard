...
jest.mock('@odh-dashboard/dashboard-build/loadManifest', () => ({
  loadManifest: jest.fn(() => null),
  getWorkspacePackages: jest.fn(),
  resetCache: jest.fn(),
}));
...
function wireLoadManifestMockForTests(): void {
  const loadBuild = jest.requireMock('@odh-dashboard/dashboard-build/loadManifest') as {
    loadManifest: jest.Mock;
    getWorkspacePackages: jest.Mock;
  };
  loadBuild.loadManifest.mockReturnValue(null);
  loadBuild.getWorkspacePackages.mockImplementation(() => {
    try {
      const raw: string | Buffer = mockExecSync('npm query .workspace --json', {
        encoding: 'utf8',
      }) as string | Buffer;
...
describe('getModuleFederationConfigs', () => {
  ...
  describe('error handling', () => {
    ...
    it('should return empty array when workspace package query fails (dashboard-build swallow errors)', () => {
      mockExecSync.mockImplementation(() => {
        throw new Error('npm query failed');
      });

      expect(getModuleFederationConfigs(true)).toEqual([]);
    });

    it('should return empty array when workspace package query returns invalid JSON', () => {
