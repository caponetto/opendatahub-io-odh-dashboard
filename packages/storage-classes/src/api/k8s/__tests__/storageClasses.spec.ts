import { k8sGetResource, k8sListResource, k8sPatchResource } from '@odh-dashboard/k8s-browser';
...
import { getStorageClasses } from '@odh-dashboard/storage-classes-shared/api/k8s/storageClasses';
import { getDefaultStorageClassConfig } from '@odh-dashboard/storage-classes-shared/concepts/storageClasses/utils';
import { getStorageClass, updateStorageClassConfig } from '../storageClasses';
...
describe('updateStorageClassConfig', () => {
  it('should add a storage class config if it does not exist', async () => {
    ...
    if (typeof actualValue === 'string') {
      ...
      throw new TypeError('Actual value is not a string');
    }
    expect(mockGetResource).toHaveBeenCalledTimes(1);
