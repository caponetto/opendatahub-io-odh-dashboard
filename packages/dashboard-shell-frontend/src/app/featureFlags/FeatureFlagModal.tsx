...
import { FeatureFlagLauncherProps } from './FeatureFlagLauncher';
...
const FeatureFlagModal: React.FC<Props> = ({
  ...
  const contents = (
    <Tabs activeKey={activeTabKey} onSelect={(_event, tabIndex) => setActiveTabKey(tabIndex)}>
      ...
      {devFlags.length > 0 && (
        <Tab eventKey={1} title={<TabTitleText>Dev Flags</TabTitleText>} data-testid="devFlagTab">
          <Content style={{ height: '400px', padding: '8px' }}>
            <Content component="p">
  ...
  return (
    <ContentModal
      ...
      dataTestId="dev-feature-flags-modal"
      variant="large"
    />
  );
};
