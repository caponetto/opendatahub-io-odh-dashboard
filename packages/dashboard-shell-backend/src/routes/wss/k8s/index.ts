/* eslint-disable @typescript-eslint/consistent-type-assertions -- WS / TLS options from Node */
...
const wsErrMsg = (err: unknown): string => (err instanceof Error ? err.message : String(err));
  ...
  fastify.get(
    ...
    (
      ...
      getDirectCallOptions(fastify, req, '').then((requestOptions) => {
        ...
        if (!cluster) {
          closeWebSocket(source, 1011, 'No kubernetes cluster configured');
          return;
        }

        const url = `${cluster.server}/${kubeUri}?${new URLSearchParams(req.query).toString()}`;
        ...
        const target = new WebSocket(url, subprotocols, {
          headers: {
            ...
            origin: req.headers.origin ?? `http://${originFallback}`,
          },
          ...
          tracked.heartbeatInterval = setInterval(() => {
            ...
            if (target.readyState === WebSocket.OPEN) {
              try {
                ...
                fastify.log.error(
                  { connectionId, error: wsErrMsg(error) },
                  `Failed to send ping: ${wsErrMsg(error)}`,
                );
              }
            }
          ...
          if (source.readyState === WebSocket.OPEN) {
            try {
              ...
              fastify.log.error(
                {
                  connectionId,
                  error: wsErrMsg(error),
                  messagesReceived: metrics.messagesReceived,
                  messagesSent: metrics.messagesSent,
                },
                `Failed to forward message to client, closing connection: ${wsErrMsg(error)}`,
              );
              close(1011, `Send failed: ${wsErrMsg(error)}`);
            }
          } else {
            fastify.log.warn(
              {
        ...
        target.on('error', (error) => {
          fastify.log.error(
            ...
            `K8s API websocket error for ${kubeUri}: ${wsErrMsg(error)}`,
          );

          close(1011, wsErrMsg(error));
        });
          ...
          waitConnection(target, () => {
            try {
              ...
              fastify.log.error(
                ...
                `Failed to forward message to K8s API: ${wsErrMsg(error)}`,
              );
            }
          }),
        );
        ...
        source.on('ping', (data) =>
          waitConnection(target, () => {
            try {
              ...
              fastify.log.debug(
                { connectionId, error: wsErrMsg(error) },
                `Failed to forward ping: ${wsErrMsg(error)}`,
              );
            }
          }),
        );
        ...
        source.on('pong', (data) =>
          waitConnection(target, () => {
            try {
              ...
              fastify.log.debug(
                { connectionId, error: wsErrMsg(error) },
                `Failed to forward pong: ${wsErrMsg(error)}`,
              );
            }
          }),
        );
        ...
        source.on('error', (error) => {
          fastify.log.error(
            ...
            `Client websocket error for ${kubeUri}: ${wsErrMsg(error)}`,
          );
          close(1011, wsErrMsg(error));
        });
        ...
        target.on('ping', (data) => {
          if (source.readyState === WebSocket.OPEN) {
            try {
              ...
              fastify.log.debug(
                { connectionId, error: wsErrMsg(error) },
                `Failed to forward ping to client: ${wsErrMsg(error)}`,
              );
            }
          }
        });
        ...
        target.on('pong', (data) => {
          if (source.readyState === WebSocket.OPEN) {
            try {
              ...
              fastify.log.debug(
                { connectionId, error: wsErrMsg(error) },
                `Failed to forward pong to client: ${wsErrMsg(error)}`,
              );
            }
          }
        });
