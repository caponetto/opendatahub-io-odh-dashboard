...
import ProjectScopedIcon from '@odh-dashboard/dashboard-foundation-frontend/components/searchSelector/ProjectScopedIcon.tsx';
import {
  ProjectScopedGroupLabel,
  ProjectScopedSearchDropdown,
} from '@odh-dashboard/dashboard-foundation-frontend/components/searchSelector/ProjectScopedSearchDropdown';
import ProjectScopedToggleContent from '@odh-dashboard/dashboard-foundation-frontend/components/searchSelector/ProjectScopedToggleContent';
import {
...
const ServingRuntimeTemplateSection: React.FC<ServingRuntimeTemplateSectionProps> = ({
  ...
  const safeProjectSpecificTemplates = React.useMemo(
    () => projectSpecificTemplates?.[0] ?? [],
    [projectSpecificTemplates],
  );
  ...
  const filterProjectScopedTemplates = React.useMemo(
    () =>
      safeProjectSpecificTemplates.filter((template) => {
        try {
          return isServingRuntimeKind(template.objects[0]);
        } catch (e) {
          return false;
  ...
  return (
    <FormGroup
      ...
      isRequired
      labelHelp={
        isProjectScoped && filterProjectScopedTemplates.length > 0 ? (
