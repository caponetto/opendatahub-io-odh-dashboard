...
import { getConfigMap } from '@odh-dashboard/dashboard-foundation-backend/envUtils';
...
export async function makeAuthenticatedHttpRequest<T = unknown>(
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = requestModule.request(requestOptions, (res: import('http').IncomingMessage) => {
      let data = '';
      ...
      res.on('end', () => {
        try {
          ...
          resolve({ data: jsonData, statusCode: res.statusCode ?? 0 });
        } catch (parseError) {
