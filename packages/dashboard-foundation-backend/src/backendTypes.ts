...
// TODO: constant-ize the x-forwarded header (read from request.headers after upstream proxy)
/** Same as FastifyRequest; oauth headers are validated at runtime, not narrowed in the generic. */
...
export type Volume = {
  name: string;
  // Kubeflow / notebook payloads may extend emptyDir arbitrarily; RecursivePartial merges require `any`.
...
import { ODH_PRODUCT_NAME } from './odhProductName';

/** Reference: https://github.com/red-hat-data-services/rhods-operator/blob/main/pkg/cluster/const.go */
export const OdhPlatformType = {
  OPEN_DATA_HUB: ODH_PRODUCT_NAME,
