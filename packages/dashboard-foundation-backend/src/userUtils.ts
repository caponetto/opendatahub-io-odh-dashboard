...
import { errorHandler, isHttpError } from './backendUtils';
...
/** Extract username field from kubernetes SelfSubjectReview response without client typings. */
const readUsernameFromSelfSubjectReviewBody = (body: unknown): string | undefined => {
  if (typeof body !== 'object' || body === null || !('status' in body)) {
    return undefined;
  }
  const statusUnknown = Reflect.get(body, 'status');
  if (
    typeof statusUnknown !== 'object' ||
    statusUnknown === null ||
    !('userInfo' in statusUnknown)
  ) {
    return undefined;
  }
  const userInfoUnknown = Reflect.get(statusUnknown, 'userInfo');
  if (typeof userInfoUnknown !== 'object' || userInfoUnknown === null) {
    return undefined;
  }
  if (!('username' in userInfoUnknown)) {
    return undefined;
  }
  const username = Reflect.get(userInfoUnknown, 'username');
...
export const getOpenshiftUser = async (
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- kube custom object response body is loosely typed on the client
...
export const getUser = async (
  ...
  const accessTokenRaw = request.headers[USER_ACCESS_TOKEN];
...
export const getUserInfo = async (
  ...
  const rbacUsernameRaw = request.headers[KUBE_RBAC_USER_HEADER];
  const rbacUsername = typeof rbacUsernameRaw === 'string' ? rbacUsernameRaw : '';
  if (rbacUsername) {
...
export const getUsernameFromSelfSubjectReview = async (
  ...
  try {
    ...
    const username = readUsernameFromSelfSubjectReviewBody(response.body);
