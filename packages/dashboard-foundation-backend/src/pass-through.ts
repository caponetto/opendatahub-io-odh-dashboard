...
export const isK8sStatus = (data: unknown): data is K8sStatus =>
  typeof data === 'object' && data !== null && Reflect.get(data, 'kind') === 'Status';
    ...
    .then(([rawData]) => {
      ...
      try {
        ...
        if (rawData.trim() === '404 page not found') {
          ...
          fastify.log.error(e, `Parsing response error: ${JSON.stringify(data)}`);
      ...
      if (isK8sStatus(parsedData)) {
        if (parsedData.status !== 'Success') {
          fastify.log.warn(
            `Unsuccessful status Object, ${JSON.stringify(
              parsedData,
              null,
              DEV_MODE ? 2 : undefined,
            )}`,
          );
