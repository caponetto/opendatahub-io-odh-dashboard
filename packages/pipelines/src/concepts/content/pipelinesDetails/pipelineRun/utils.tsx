...
import { DateTimeKF, RuntimeConfigParamValue } from '@odh-dashboard/pipelines/concepts/kfTypes';
...
export const checkPodContainersStatus = (
  ...
  return {
    podInitializing: hasState && 'waiting' in state && !!state.waiting,
    running: hasState && 'running' in state && !!state.running,
    completed: hasState && 'terminated' in state && !!state.terminated,
  };
};
