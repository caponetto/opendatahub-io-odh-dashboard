jest.mock('@odh-dashboard/model-registry/concepts/modelRegistry/utils/updateTimestamps', () => ({
  bumpRegisteredModelTimestamp: jest.fn().mockResolvedValue(undefined),
}));
...
import {
  ...
  proxyGET,
  proxyPATCH,
} from '@odh-dashboard/dashboard-foundation-frontend/api/proxyUtils';
import { mockRegisteredModel } from '@odh-dashboard/test-mocks/mockRegisteredModel';
import { MODEL_REGISTRY_API_VERSION } from '@odh-dashboard/model-registry/concepts/modelRegistry/const';
import {
  ModelArtifactState,
  ModelState,
...
describe('createModelVersionForRegisteredModel', () => {
  it('should call proxyCREATE and handleModelRegistryFailures to create model version for a model', async () => {
    await expect(
