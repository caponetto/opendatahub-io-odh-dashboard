...
export const createModelRegistryAndCredentials = async (
  ...
  let registryToCreate = modelRegistry;
  if (newDatabaseCACertificate) {
    const sslConfig = registryToCreate.spec.mysql?.sslRootCertificateConfigMap;
    if (!sslConfig) {
      throw new Error('MySQL SSL root certificate config map is required for CA certificate');
    }
    ...
    const newCACertificateConfigMap = {
      name: newConfigMap.metadata?.name ?? '',
      key: Object.keys(newConfigMap.data || {})[0],
    };

    if (registryToCreate.spec.mysql) {
      registryToCreate = {
        ...registryToCreate,
        spec: {
          ...registryToCreate.spec,
          mysql: {
            ...registryToCreate.spec.mysql,
            sslRootCertificateConfigMap: newCACertificateConfigMap,
          },
        },
      };
    }
  ...
  const createBoth = async (dryRun = false) => {
    const dbSpec = getDatabaseSpec(registryToCreate);
    const newSecret =
...
export const getDatabasePassword = async (
  ...
  if (!secret?.data || !passwordDataKey) {
    return undefined;
  }
...
const deleteDatabasePasswordSecret = async (
  ...
  const secretName = existingSecret.metadata?.name;
  if (!secretName) {
    return;
  }
  ...
  if (databasePassword) {
    ...
    await fastify.kube.coreV1Api.patchNamespacedSecret(
      ...
      modelRegistryNamespace,
      {
        data: {
          ...secret.data,
          ...(passwordDataKey ? { [passwordDataKey]: base64encode(databasePassword) } : {}),
        },
      },
      undefined,
      dryRun ? 'All' : undefined,
...
export const patchModelRegistryAndUpdateCredentials = async (
  ...
  const patchBodyForRequest = patchBody;
  if (newDatabaseCACertificate) {
    const sslConfigName = patchBodyForRequest.spec?.mysql?.sslRootCertificateConfigMap?.name;
    if (!sslConfigName) {
      throw new Error('MySQL SSL root certificate config map name is required for CA certificate');
    }
    const newConfigMap = await createConfigMapForCACertficate(
      fastify,
      modelRegistryNamespace,
    ...
    if (patchBodyForRequest.spec?.mysql) {
      // eslint-disable-next-line no-param-reassign
      patchBodyForRequest.spec.mysql.sslRootCertificateConfigMap = newCACertificateConfigMap;
    }
  }
