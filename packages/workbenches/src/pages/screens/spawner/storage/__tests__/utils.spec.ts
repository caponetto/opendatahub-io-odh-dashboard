...
import { getPossibleStorageClassAccessModes } from '@odh-dashboard/storage-classes-shared/concepts/storageClasses/utils';
import { mockStorageClasses } from '@odh-dashboard/test-mocks/mockStorageClasses';
import { MOUNT_PATH_PREFIX } from '@odh-dashboard/workbenches/pages/screens/spawner/storage/const';
import { MountPathFormat } from '@odh-dashboard/workbenches/pages/screens/spawner/storage/types';
import {
  useCreateStorageObject,
  useMountPathFormat,
  validateMountPath,
} from '@odh-dashboard/workbenches/pages/screens/spawner/storage/utils';

jest.mock('@odh-dashboard/workbenches/pages/screens/spawner/storage/useDefaultPvcSize', () => ({
  __esModule: true,
  default: jest.fn().mockReturnValue('1Gi'), // Set the default PVC size to 1Gi
}));

jest.mock('@odh-dashboard/dashboard-foundation-frontend/concepts/k8s/utils', () => ({
...
describe('getPossibleStorageClassAccessModes', () => {
  ...
  it('returns RWO as the only supported access mode if config accessModeSettings is empty', () => {
    const storageClass = {
